// ── HARDCODED AOI — original confirmed extent ──────────────────────────
//
// MEMORY TAG: GalaSat_v2_HardcodedAOI_476pts
// SOURCE: claude.ai chat "Improving GalaSat accuracy with cloud masking"
//         https://claude.ai/chat/0e1e385b-f524-43ed-92f1-885568da9617
//         pasted attachment, 373 lines, 16,729 bytes
//         Account 2 (Teqami's), discovered 2026-04-25 cross-account sweep
// EXTRACTED: 2026-04-25
//
// LINEAGE NOTE: Self-labels GalaSat v2.0 in legend. Distinguishing features
// vs the trainingPoints branch:
//   - AOI = HARDCODED Rectangle [-3.1, 5.4, -0.9, 7.2] (not dynamic from polygon bounds)
//   - Trains on NEW asset infn8vzn_mercury_training_476 (476 pts, between 340 and 752)
//   - Pulls BOTH class 1 AND class 0 directly from the asset (handles string and number class IDs)
//   - 150 auto-clean forest reference points (vs 278 in 274/259)
//   - Adds validPixelMask = s2dry.select('B4').mask() — explicit valid-pixel mask
//   - Original Sentinel-2 dry composite (not the Landsat asset variant of v2.2)
//
// NOTE: Whitespace partially collapsed during DOM extraction.

var AOI = ee.Geometry.Rectangle([-3.1, 5.4, -0.9, 7.2]);

// ── ASSETS ─────────────────────────────────────────────────────────────
var trainingPoints = ee.FeatureCollection(
  'projects/galamsey-monotoring/assets/infn8vzn_mercury_training_476'
);
...
var s2dry = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
  .filterBounds(AOI).filterDate('2025-11-01','2026-02-28')
...
// Both class types (handles asset with mixed string/number class field)
var class1Points = trainingPoints.filter(ee.Filter.eq('class', 1))
  .merge(trainingPoints.filter(ee.Filter.eq('class', '1')))
  .map(function(f){ return f.set('class', 1); });
var class0Points = trainingPoints.filter(ee.Filter.eq('class', 0))
  .merge(trainingPoints.filter(ee.Filter.eq('class', '0')))
  .map(function(f){ return f.set('class', 0); });

print('── Training Asset Check ───────────────');
print('Class 1 points in asset:', class1Points.size());
print('Class 0 points in asset:', class0Points.size());

var class1Sampled = imageStack.sampleRegions({
  collection: class1Points, properties: ['class'], scale: 10, tileScale: 8
});
var class0Sampled = imageStack.sampleRegions({
  collection: class0Points, properties: ['class'], scale: 10, tileScale: 8
});

var autoClean = imageStack.updateMask(NDVI.gt(0.6)).sample({
  region: AOI, scale: 30, numPixels: 150, seed: 42, geometries: true
}).map(function(f){ return f.set('class', 0); });

var allSamples = class1Sampled.merge(class0Sampled).merge(autoClean)
...
print('Class 1 sampled:', class1Sampled.size());
print('Class 0 sampled:', class0Sampled.size());
print('Auto-clean forest:', autoClean.size());
...
var validPixelMask = s2dry.select('B4').mask();
var finalMask = urbanMask.and(roadMask).and(validPixelMask);
...
function getYearTurbidHa(year) {
  ...
  var merged = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
    .filterBounds(AOI).filterDate(s,e).filter(ee.Filter.lt('CLOUD_COVER',30)).map(maskL8)
    .merge(ee.ImageCollection('LANDSAT/LC09/C02/T1_L2')
      .filterBounds(AOI).filterDate(s,e).filter(ee.Filter.lt('CLOUD_COVER',30)).map(maskL9));
  return ee.Algorithms.If(merged.size().gte(3),
    ee.Number(merged.median().clip(AOI)
      .normalizedDifference(['SR_B3','SR_B6']).rename('MNDWI').gt(0)
      .and(merged.median().clip(AOI)
        .normalizedDifference(['SR_B4','SR_B3']).rename('NDTI').gt(0.05))
...
Map.addLayer(s2dry, {bands:['B4','B3','B2'],min:0,max:0.3}, 'True Colour 2026');
Map.addLayer(forest, {palette:['1A5C1A']}, 'Forest Cover');
Map.addLayer(permanent_water, {palette:['#003087']}, 'Permanent Water');
...
Map.addLayer(trainingPoints, {color:'FF6600'}, 'Training Points (476)');
Map.addLayer(basin_fc.style({color:'FFFFFF',fillColor:'00000000',width:2}), {}, 'River Basin Boundaries');
...
legend.add(makeRow('#FF6600','Training Points (476)'));
legend.add(makeRow('#FFFFFF','Pilot AOI Boundary | River Basin Boundaries'));
legend.add(ui.Label({
  value:'Sentinel-2 SR | SCL cloud mask | Dual composite | EPSG:4326',
  style:{fontSize:'10px',color:'#555555',margin:'6px 0 0 0'}
}));
legend.add(ui.Label({
  value:'Sources: Joy News | Al Jazeera | PMC 2024 | GWCL | Global Mining Watch | JRC',
...
Export.image.toDrive({
  image:flood_zone,
  description:'INFN8VZN_Flood_Zone',
  folder:'GalaSat',
  fileNamePrefix:'infn8vzn_flood_zone',
