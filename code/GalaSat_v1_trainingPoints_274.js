// ============================================================
// GalaSat v1.0 — trainingPoints architecture, 274-line build
//
// MEMORY TAG: GalaSat_v1.0_trainingPoints_274
...
//         pasted attachment, 274 lines
// EXTRACTED: 2026-04-25
//
// LINEAGE NOTE: trainingPoints branch member. Adds a toggleable cloud
// coverage layer (computes SCL classes 3/8/9/10 max across wet-season
// composite) — first appearance of cloud-as-a-layer. Also: Tano basin
// shifted from [-3.20, 6.50, -2.00, 7.80] to [-2.80, 6.60, -2.10, 7.55]
// (smaller footprint than the 506/311 variants).
//
...
var s2dry = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
  ...
  .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',20)).map(maskS2clouds).median().clip(AOI);
...
// ── CLOUD COVERAGE LAYER — toggleable (NEW IN THIS BUILD) ──────────────
var cloudLayer = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
  .filterBounds(AOI)
  .filterDate('2025-06-01','2025-09-30')
  .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',30))
  .map(function(img) {
    var scl = img.select('SCL');
    return scl.eq(8).or(scl.eq(9)).or(scl.eq(10)).or(scl.eq(3)).selfMask();
  })
  .max()
  .clip(AOI)
  .rename('clouds');

// ── BASIN GEOMETRIES (Tano basin smaller than 506/311 variants) ────────
...
Map.addLayer(exposedCommunitiesRaster, {palette:['FF00FF']}, 'Communities within 5km');
Map.addLayer(cloudLayer, {palette:['#C0C0C0'], opacity:0.6}, 'Cloud Coverage (toggle)', false);
