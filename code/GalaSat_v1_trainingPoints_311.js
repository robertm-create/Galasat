// ============================================================
// GalaSat v1.0 — trainingPoints architecture, 311-line build
//
// MEMORY TAG: GalaSat_v1.0_trainingPoints_311
...
//         pasted attachment, 311 lines
// EXTRACTED: 2026-04-25
//
// LINEAGE NOTE: trainingPoints branch member. Adds SAR (VV/VH/VVVH_ratio),
// GLCM texture features (B8_contrast / B8_ent / B8_var), and slope/elevation
// from a DEM as RF input bands. Total bands = 24.
//
// IMPORTANT — POSSIBLE INCOMPLETE: The captured code references variables
// `VV`, `VH`, `VVVH`, `glcm`, `slope`, `elevation` in the imageStack
// definition, but their preceding declarations are not present in the
// captured DOM text. They may have been collapsed during whitespace
// flattening or the source itself may be a working draft with missing
// upstream defs. Reconstruct the SAR + GLCM + DEM blocks from the
// 506-line variant + the 274/259/210 siblings before running.
//
...
// ── FEATURE STACK (24 bands incl. SAR + GLCM + DEM) ───────────────────
// NOTE: VV / VH / VVVH / glcm / slope / elevation referenced below but
// their definitions are missing from the captured DOM. See header note.
var imageStack = s2dry.unmask(0).select(['B2','B3','B4','B5','B6','B7','B8','B8A','B11','B12'])
  .addBands(NDVI).addBands(NDWI).addBands(MNDWI).addBands(NDTI).addBands(BSI).addBands(IOR)
  .addBands(VV).addBands(VH).addBands(VVVH)
  .addBands(glcm)
...
Map.addLayer(worldPop, {min:0, max:200, palette:['#FFFFE0','#FFA500','#FF0000']}, 'Population Density', false, 0.7);
