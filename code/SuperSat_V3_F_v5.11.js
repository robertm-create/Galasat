...
// (header is stale — see footer print: this is actually v5.11)
// INFN8VZN | Ashanti Region Mercury Contamination Detection
// RF now trains on full polygon areas — not centroid points
// ============================================================
//
// MEMORY TAG: (unlabeled — transitional)  | INTERNAL VERSION: v5.11
// SUBMITTED IN CHAT AS: "SuperSat_V3_F"  (label is wrong — code is v5.11, NOT V3 / v5.16)
// SOURCE: claude.ai project "Technical - Repository"
//         chat "SuperSat_V1 code tagging" (Apr 20, 2026)
//         pasted attachment, 607 lines, 32.45 KB
// EXTRACTED: 2026-04-25
//
// LINEAGE: Sits BETWEEN SuperSat_V2 (v5.4) and SuperSat_V3 (v5.16). Five minor versions short of V3.
//   - SAR + HydroSHEDS already integrated (V3 architecture)
//   - ownMiningProximity STILL in RF chain (V3 drops this — regression)
//   - mineS2 minPx still 12 (V3 raises to 50)
//   - Time series cloud filter still 30% (V3 raises to 75%)
//   - Legend accuracy stale at "82% / Kappa 0.733 / 805 polygons" (V2 numbers — never re-validated)
//   - Cumulative mining disturbance NOT YET added (V3-only)
//
// UNIQUE TO THIS BUILD: HLS backup comment block (NASA/HLS/HLSL30/v002) — does not appear in any other build.
// Per chat: "Should be activated and tested in V4 given the cloud cover problem in wet season composites."
//
...
// ── SENTINEL-1 SAR — RADAR (sees through clouds) ───────────
...
print('── SAR ────────────────────────────────');
print('S1 dry season images loaded (radar)');
...
// ── RULE-BASED DETECTORS  (v5.11: minPx still 12 — V3 raises to 50) ───
function mineS2(img) {
  var b2=img.select('B2'),b3=img.select('B3'),b4=img.select('B4');
  var b8=img.select('B8'),b8a=img.select('B8A'),b11=img.select('B11');
...
// Single combined mining activity layer — latest detection (2026) with tricolor palette
...
// Tighter thresholds: must be water AND have high turbidity signal
...
// ── HYDROSHEDS — Scientific river network + watersheds ─────
// Replaces hand-drawn basin polygons with validated data
...
// ── RF CLASSIFIER — TRAINED ON POLYGON AREAS  (20 bands incl. SAR) ───
...
// Add SAR bands to image stack for RF training
...
// Mining + urban: sample from polygon areas (controlled size)
// Scrubland: use KML centroid points — scrubland polygons are too large to sample
...
// Scrubland: sample at 500m resolution to get centroids effectively (gets ~1 sample per polygon)
var sSampled = imageStack.sampleRegions({collection: scrubPolys,  properties: ['c'], scale: 500, tileScale: 4});
...
print('Total samples (3600 max):', allSampled.size());
...
// RF mining output — v5.11 RETAINS ownMiningProximity in chain (V3 drops this)
var rfMining = rfClass.eq(1)
  ...
  .updateMask(ownMiningProximity)   // v5.11 KEEPS this; V3 removes it
  .selfMask();
...
// ── TIME SERIES CHART  (v5.11: cloud filter still 30% — V3 raises to 75%) ──
...
// Single combined mining activity layer (tricolor — toggleable)
Map.addLayer(miningActivity, mineViz, 'Mining Activity 2026 (tricolor)', true);

// Individual years — off by default, toggle on to compare
...
// ── HLS (Harmonized Landsat Sentinel-2) — RISK MITIGATION ───
// NASA/HLS/HLSL30/v002 available in GEE as backup optical source
// Activated if primary S2/L8 composites have insufficient coverage
// Referenced in application Q29 risk mitigation section
// var hlsBackup = ee.ImageCollection('NASA/HLS/HLSL30/v002')
//   .filterBounds(box).filterDate('2025-10-01','2026-03-31')
//   .filter(ee.Filter.lt('CLOUD_COVERAGE',30));

print('GalaSat v5.11 — Build complete');
print('RF: SAR+S2 fusion | 20 bands | OSM road mask | 8km proximity');

// ── LEGEND  (note: accuracy stale at V2 numbers — never re-validated) ──
...
legend.add(ui.Label({value:'GalaSat v5.11 — INFN8VZN',style:{fontWeight:'bold',fontSize:'12px',margin:'0 0 4px 0',color:'#000000',backgroundColor:'#F0C0E0',padding:'2px 5px'}}));
...
// STALE — these numbers are V2's, not v5.11's. v5.11 was committed before re-validation.
