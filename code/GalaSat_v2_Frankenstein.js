...
//
// MEMORY TAG: GalaSat_v2.0_Final_Frankenstein
// SOURCE: claude.ai chat "Google grant application continuation"
//         https://claude.ai/chat/afc504e4-643d-40f4-9603-dc611561599c
//         pasted attachment, 659 lines, 30,693 bytes
// EXTRACTED: 2026-04-25
//
// LINEAGE NOTE: Parallel build, NOT in the SuperSat lineage table established
// in chat 97720585 ("SuperSat_V1 code tagging"). Self-labeled "Frankenstein"
// — explicitly a hybrid of the wide-AOI training_340 baseline + V5.20
// composite settings + V5.20 three-class polygon RF + V3 layer-toggle UX
// + clean legend from older code. Polygon count 628 matches SuperSat_V3.
//
// NOTE: Whitespace partially collapsed during DOM extraction. Logic and
// semicolons intact — runnable. Run a JS formatter if you want canonical indentation.
...
// Permanent water mask - removes reservoirs and lakes from all detection layers
// Threshold 90: only removes truly permanent water, not seasonal river corridors
var permWater = ee.Image('JRC/GSW1_4/GlobalSurfaceWater')
  .select('occurrence').gt(90).selfMask();
var permWaterBuffered = permWater.focal_max(500, 'circle', 'meters');
var permWaterMask = permWaterBuffered.unmask(0).not();
...
var rfRaw = rfClass.eq(1)
  ...
  .updateMask(ndviImg.lt(0.68));
var rfMining = rfRaw
  .updateMask(rfRaw.connectedPixelCount(200, true).gte(6))
...
var m22=mineS2(s2_22),m24=mineS2(s2_24),m25=mineS2(s2_25);
var m26raw=mineS2(s2_26);
var m26=m26raw.updateMask(permWaterMask).selfMask();
...
var cumulativeMining = ml14.unmask(0).gt(0)
  ...
  .or(m25.unmask(0).gt(0)).or(m26raw.unmask(0).gt(0))
  .selfMask()
  .updateMask(permWaterMask);
...
var permWater2 = jrcOccurrence.gt(75).selfMask().clip(AOI);  // re-declared in source
...
var totalPopDict = worldPop.reduceRegion({
  reducer: ee.Reducer.sum(), geometry: AOI, scale: 100, maxPixels: 1e13, bestEffort: true
});

var miningBufferGeom = globalMines.map(function(f){
  return f.buffer(2000);
}).union(ee.ErrorMargin(100)).geometry();

var exposedPopDict = worldPop.reduceRegion({
  reducer: ee.Reducer.sum(), geometry: miningBufferGeom, scale: 100, maxPixels: 1e13, bestEffort: true
});

print('WorldPop Population Estimates (2020)');
print('Total population in AOI:', totalPopDict);
print('Population within 2km of confirmed mining:', exposedPopDict);
...
Map.addLayer(permWater2,{palette:['#003087']},'Permanent Water',false);
...
Export.image.toDrive({
  image:rfMining,
  description:'INFN8VZN_RF_Mining_2025',
  folder:'GalaSat',
  fileNamePrefix:'infn8vzn_rf_mining_2025',
...
Export.image.toDrive({
  image:cumulativeMining,
  description:'INFN8VZN_Cumulative_Mining_2014_2026',
  folder:'GalaSat',
  fileNamePrefix:'infn8vzn_cumulative_mining_2014_2026',
  region:AOI, scale:30, crs:'EPSG:4326', maxPixels:1e13, fileFormat:'GeoTIFF'
});
